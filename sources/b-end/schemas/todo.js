...
const todoTypeDefs = `#graphql
  ...
  type Query {
    ...
    todoList: TodoResponse
		...
		# New One !
		# todoList from database
		todoListFromDb: TodoMongoDbResponse
  }
...
const todoResolvers = {
	Query: {
		...
		// ? Using the third argument to access the "global variable"
		todoListFromDb: async (_, __, contextValue) => {
			// ? Get the db value
			const { db } = contextValue;

			// ? Use db to do query !
			const todosQuery = await db.collection("Todos").find({}).toArray();
